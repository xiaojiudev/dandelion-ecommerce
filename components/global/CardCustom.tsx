...
export default function CardCustom(props: any) {
    ...
    const result = props?.data?.map((item: ProductDetails) =>
    (
        <Link href={`/product/${item?.id}`} key={`${item?.id}`} target='' className="relative min-w-[180px] max-w-[200px] h-[285px] block bg-white text-gray-900 shadow-md rounded-3xl p-2 cursor-pointer hover:-translate-y-1 transition-all select-none">
            {props.isLoading ? (
                <>
